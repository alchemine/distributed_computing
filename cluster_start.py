...
if __name__ == '__main__':
    ...
    asyncio.run(run_client_background(config_scheduler, cmd_scheduler, timeout=1))


    ## 5. Connect workers to scheduler
    config_workers = []
    cmd_workers    = []
    for config in [config for id, config in configs.items() if id in args.worker.split()]:
        for gpu in range(int(config['gpus'])):
            log_path = abspath(join(dirname(__file__), f'worker--gpu{gpu}.log'))
            cmd_worker = f"source /opt/conda/bin/activate rapids; \
                           export CUDA_VISIBLE_DEVICES={gpu}; \
                           rm {log_path}; \
                           nohup dask-worker {config_scheduler['host']}:{config_scheduler['scheduler_port']} --nthreads 1 --nprocs 3 --memory-limit='20 GiB' --no-reconnect >> {log_path} 2>&1"
            config_workers.append(config)
            cmd_workers.append(cmd_worker)
    asyncio.run(run_clients_background(config_workers, cmd_workers, timeout=5))
